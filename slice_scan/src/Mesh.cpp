...
bool Mesh::ReadFromObj(std::string file_name) {
  std::string mtl_file_name(file_name);
  mtl_file_name.replace(mtl_file_name.size() - 3, 3, "mtl");

  std::ifstream mtl_file;
  mtl_file.open(mtl_file_name);
  if (mtl_file.is_open()) {
    std::cout << "Reading mtl: "<<mtl_file_name << std::endl;
    std::string s;
    std::string m_name;
...
void SliceScan(Mesh &mesh, double r_grid, double r_slice, int thickness) {
  ...
  for (int i = 0; i < mesh.face_list.size(); ++i) {
    ...
    int r = 255 * mesh.material_map[mtl_name].Ka.r;
    int g = 255 * mesh.material_map[mtl_name].Ka.g;
    int b = 255 * mesh.material_map[mtl_name].Ka.b;
  ...
  for (int i = 0; i < images.size(); ++i) {
    ...
    oss << "scaned_layer_" << i << ".jpg";
    std::string file_name(oss.str());
    cv::imwrite(file_name, *images[i]);
  }
}