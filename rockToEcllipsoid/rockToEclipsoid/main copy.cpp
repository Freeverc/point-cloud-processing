...
int main(int argc, char *argv[])
{
  ...
  for (int i = 0; i < faces_ref.size(); ++i)
  {
    Mesh::Vertex_index v0(faces_ref[i][0]);
    Mesh::Vertex_index v1(faces_ref[i][1]);
    Mesh::Vertex_index v2(faces_ref[i][2]);
    mesh.add_face(v0, v1, v2);
  }

  const unsigned int nb_iterations = (argc > 2) ? std::atoi(argv[2]) : 1;

  std::cout << "Verteces : " << mesh.vertices().size() << std::endl;
  ...
  CGAL::IO::write_OBJ("./mesh_smoothed.obj", mesh);
